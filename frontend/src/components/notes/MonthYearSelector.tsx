...
export function MonthYearSelector({
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape') { setIsMonthOpen(false); setIsYearOpen(false); }
    };
  ...
  const relativeLabel = (() => {
    ...
    const abs = Math.abs(monthDiff), dir = monthDiff > 0 ? 'ahead' : 'ago';
    if (abs === 1) return monthDiff > 0 ? 'Next month' : 'Last month';
    if (abs < 12) return `${abs} months ${dir}`;
    const y = Math.floor(abs / 12), m = abs % 12;
    return m === 0 ? `${y} ${y === 1 ? 'year' : 'years'} ${dir}` : `${y}y ${m}m ${dir}`;
  })();
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="p-2.5 rounded-xl" style={{ backgroundColor: 'var(--monarch-orange-light)' }}>
            <Calendar size={20} style={{ color: 'var(--monarch-orange)' }} />
          </div>
          <button
            type="button"
            onClick={handlePrevMonth}
            className="p-2 rounded-lg hover:bg-[var(--monarch-bg-hover)] transition-colors icon-btn-hover"
        ...
        <div className="flex flex-col items-center">
          <div className="flex items-center gap-1">
            <div ref={monthRef} className="relative">
              ...
              {isMonthOpen && (
                <div
                  className="absolute top-full left-1/2 -translate-x-1/2 mt-1 py-1 rounded-lg shadow-lg min-w-[140px]"
                  style={{
            ...
            <div ref={yearRef} className="relative">
              ...
              {isYearOpen && (
                <div
                  className="absolute top-full left-1/2 -translate-x-1/2 mt-1 py-1 rounded-lg shadow-lg min-w-[100px] max-h-[200px] overflow-y-auto"
                  style={{
                    backgroundColor: 'var(--monarch-bg-card)',
                    border: '1px solid var(--monarch-border)',
                    zIndex: 'var(--z-index-dropdown)',
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="w-[45px]" aria-hidden="true" />
        </div>
      </div>
    </div>
  );
