/**
 * Self-Contained OAuth Authorization Page
 *
 * Renders a branded HTML page where the user enters their Eclosion subdomain
 * to authorize IFTTT access. Follows the tunnel-gate/otp-page.ts pattern:
 * all CSS/JS inline, light/dark mode support.
 *
 * Flow:
 * 1. User enters their Eclosion subdomain
 * 2. Page POSTs to /oauth/authorize with subdomain, state, redirect_uri
 * 3. Worker validates subdomain + OTP configured, returns redirect URL
 * 4. Page redirects to tunnel URL where tunnel-gate handles OTP
 * 5. After OTP, Flask serves the approval page
 */

export function generateAuthorizePage(state: string, redirectUri: string): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
...
.input-group{display:flex;align-items:center;margin-bottom:16px;border:2px solid var(--border);border-radius:8px;overflow:hidden;transition:border-color .15s}
.input-group:focus-within{border-color:var(--primary)}
.subdomain-input{flex:1;padding:14px;background:var(--bg-input);border:none;font-size:15px;color:var(--text);outline:none;font-family:inherit}
.subdomain-input::placeholder{color:var(--text-muted)}
.input-suffix{padding:14px;background:var(--bg-page);color:var(--text-muted);font-size:14px;font-weight:500;border-left:1px solid var(--border);white-space:nowrap}
...
<div class="card">
  ...
  <h1>Connect Eclosion to IFTTT</h1>
  <p class="desc">Enter your Eclosion subdomain to get started. You'll verify your identity before authorizing.</p>

  <div id="error" class="error-msg"></div>

  <label class="field-label">Your Eclosion Subdomain</label>
  <div class="input-group">
    <input type="text" id="subdomain" class="subdomain-input" placeholder="my-eclosion" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false">
    <span class="input-suffix">.eclosion.me</span>
  </div>

  <button id="authorize-btn" class="btn" type="button">Continue</button>

  <p class="consent">You'll verify ownership via email code, then review permissions. <a href="https://eclosion.app/docs" target="_blank">Learn more</a></p>
</div>
...
(function(){
  ...
  btn.addEventListener('click', async function() {
    ...
    if (!subdomain) {
      showError('Please enter your Eclosion subdomain.');
      return;
    }
    if (subdomain.length < 3) {
      showError('Subdomain must be at least 3 characters.');
      return;
    }
    ...
    try {
      var resp = await fetch('/oauth/authorize', {
        method: 'POST',
      ...
      if (!resp.ok) {
        showError(data.error || 'Authorization failed. Please try again.');
        btn.disabled = false;
